<?php
  ...
  $pdf=json_decode($_GET["pdf"]);
	...
	$sqld="select * from t_sesiones order by id_sesion desc limit 1 ";
  ...
  $result=$conexion->query($sqlb);
    ...
    <header class="clearfix">
      ...
      foreach ($datosd as $datosd) 
        {
          $html.='<tr><strong>Fechas de corte:</strong> Fecha de inicio:'.$datosd["fecha_i"].' _____fecha de cierre:'.$datosd["fecha_c"].'';
        }
    $html.='</div>
    </header>
    <main>
